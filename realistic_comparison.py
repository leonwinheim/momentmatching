...
model = GMN.GaussianMixtureNetwork(layers,act_func,gm_comp_pre,gm_comp_post,moments_pre,moments_post,a_relu,peak=True)
print("Model initialized.")

x = np.array([[2.0]])   # Example input value

model.compare_sample_moments_forward(x) #Perform all forward passes so the moments are stored
...
for i in range(moments_post):
    ...
    #print(f"  Relative Error (MM vs EM): {100 * abs((predictive_moments_mm[i] - predictive_moments_em[i]) / predictive_moments_em[i]):.4f} %")
...
plt.bar(x-width, 1, width, label='Samples', color='green', alpha=0.6)
plt.bar(x , 1+0.01*rel_error_mm_samples, width, label='Moment Matching', color='blue', alpha=0.6)
plt.bar(x + width, 1+0.01*rel_error_em_samples, width, label='EM', color='red', alpha=0.6)
plt.xlabel('Moments')
plt.ylabel('Value')
plt.title('Comparison of Predictive Moments, Samples, and EM')
